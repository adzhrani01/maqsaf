...
mixin PagingMixin<T> {
  ...
  void initPageRequestListener(BuildContext context,PagingController<int, T> pagingController) {
    if(!pagingController.hasListeners) {
      pagingController.addPageRequestListener((int? pageKey) async {
        ...
        await getItems(context, skipCount: (pageKey??0)*10);
      });
    }
  }
  ...
  Future<void> getItems(BuildContext context, {required int? skipCount});
  // int? getNextPage(){
  //   if(baseModel==null) {
  //     return null;
  //   }
  ...
  // }
  int? getNextPage(PagingController<int, T> pagingController){
    if(baseModel==null) {
      return null;
    }
    int totalItems = (pagingController.itemList?.length ?? 0 )+1;
    int remainingPages = (totalItems / _pageSize).ceil();
  ...
  void handlePageData(
    ...
    if (isLastPage) {
      ...
      final nextPageKey = getNextPage(pagingController);
